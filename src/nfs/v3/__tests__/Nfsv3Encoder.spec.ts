...
describe('Nfsv3Encoder', () => {
  ...
  describe('GETATTR', () => {
    test('encodes and decodes GETATTR request', () => {
      ...
      expect(decoded.object.data.uint8).toEqual(fhData);
    });
  ...
  describe('SETATTR', () => {
    test('encodes and decodes SETATTR request', () => {
      ...
      const request = new msg.Nfsv3SetattrRequest(new structs.Nfsv3Fh(new Reader(fhData)), sattr, guard);
  ...
  describe('ACCESS', () => {
    test('encodes and decodes ACCESS request', () => {
      const fhData = new Uint8Array([1, 2, 3, 4]);
      const request = new msg.Nfsv3AccessRequest(new structs.Nfsv3Fh(new Reader(fhData)), 0x1f);
  ...
  describe('READ', () => {
    ...
    test('encodes and decodes READ response', () => {
      ...
      expect(decoded.resok!.data.uint8).toEqual(data);
    });
  });
  ...
  describe('WRITE', () => {
    test('encodes and decodes WRITE request', () => {
      ...
      expect(decoded.data.uint8).toEqual(data);
    });
  });
  ...
  describe('CREATE', () => {
    test('encodes and decodes CREATE request with UNCHECKED mode', () => {
      const fhData = new Uint8Array([1, 2, 3, 4]);
      const dirOpArgs = new structs.Nfsv3DirOpArgs(new structs.Nfsv3Fh(new Reader(fhData)), 'newfile.txt');
      const mode = new structs.Nfsv3SetMode(true, 0o100644);
      const uid = new structs.Nfsv3SetUid(false);
      const gid = new structs.Nfsv3SetGid(false);
    ...
    test('encodes and decodes CREATE request with EXCLUSIVE mode', () => {
      ...
      const how = new structs.Nfsv3CreateHow(Nfsv3CreateMode.EXCLUSIVE, undefined, new Reader(verf));
      const request = new msg.Nfsv3CreateRequest(dirOpArgs, how);
      const encoded = encoder.encodeMessage(request, Nfsv3Proc.CREATE, true);
      const decoded = decoder.decodeMessage(new Reader(encoded), Nfsv3Proc.CREATE, true) as msg.Nfsv3CreateRequest;
      expect(decoded).toBeInstanceOf(msg.Nfsv3CreateRequest);
      ...
      expect(decoded.how.verf?.uint8).toEqual(verf);
    });
  });
  ...
  describe('MKDIR', () => {
    test('encodes and decodes MKDIR request', () => {
      const fhData = new Uint8Array([1, 2, 3, 4]);
      const dirOpArgs = new structs.Nfsv3DirOpArgs(new structs.Nfsv3Fh(new Reader(fhData)), 'newdir');
  ...
  describe('SYMLINK', () => {
    test('encodes and decodes SYMLINK request', () => {
      const fhData = new Uint8Array([1, 2, 3, 4]);
      const dirOpArgs = new structs.Nfsv3DirOpArgs(new structs.Nfsv3Fh(new Reader(fhData)), 'mylink');
  ...
  describe('REMOVE', () => {
    test('encodes and decodes REMOVE request', () => {
      const fhData = new Uint8Array([1, 2, 3, 4]);
      const dirOpArgs = new structs.Nfsv3DirOpArgs(new structs.Nfsv3Fh(new Reader(fhData)), 'file-to-remove.txt');
  ...
  describe('RENAME', () => {
    test('encodes and decodes RENAME request', () => {
      ...
      const fromArgs = new structs.Nfsv3DirOpArgs(new structs.Nfsv3Fh(new Reader(fromFh)), 'oldname.txt');
      const toArgs = new structs.Nfsv3DirOpArgs(new structs.Nfsv3Fh(new Reader(toFh)), 'newname.txt');
  ...
  describe('READDIR', () => {
    ...
    test('encodes and decodes READDIR response', () => {
      ...
      const resok = new msg.Nfsv3ReaddirResOk(postOpAttr, new Reader(cookieverf), dirList);
  ...
  describe('COMMIT', () => {
    test('encodes and decodes COMMIT request', () => {
      const fhData = new Uint8Array([1, 2, 3, 4]);
      const request = new msg.Nfsv3CommitRequest(new structs.Nfsv3Fh(new Reader(fhData)), BigInt(0), 4096);

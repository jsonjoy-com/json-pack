...
class ClientRecord {
  constructor(
    /** Client verifier from SETCLIENTID. */
    ...
    /** SETCLIENTID confirmation verifier. */
    public readonly setclientidConfirm: Uint8Array,
    /** Whether this record is confirmed. */
...
export class Nfsv4OperationsNode implements Nfsv4Operations {
  ...
  protected clientRecords: Map<string, ClientRecord> = new Map();
  ...
  public async SETCLIENTID(
    ...
    const existingConfirmed = Array.from(this.clientRecords.values()).find(
    ...
    if (existingConfirmed) {
      ...
      if (verifierMatch) {
        ...
        this.clientRecords.set(unconfirmedKey, newRecord);
  ...
  public async SETCLIENTID_CONFIRM(
    ...
    const unconfirmedRecord = Array.from(this.clientRecords.entries()).find(
      ([key, record]) => !record.confirmed && record.clientid === clientid,
    );
    if (!unconfirmedRecord) {
      const confirmedRecord = Array.from(this.clientRecords.values()).find(
        (record) =>
          record.confirmed &&
    ...
    const oldConfirmed = Array.from(this.clientRecords.entries()).find(
      ([key, r]) => r.confirmed && Buffer.from(r.clientIdString).toString('hex') === clientIdStringKey,
    );
    if (oldConfirmed) {
      this.clientRecords.delete(oldConfirmed[0]);
    }
    ...
    this.clientRecords.delete(unconfirmedKey);
    this.clientRecords.set(confirmedKey, confirmedRecord);
    return new msg.Nfsv4SetclientidConfirmResponse(Nfsv4Stat.NFS4_OK);
  }
